...
class Organisasi extends CI_Controller
{
    ...
    public function insert()
    {
        ...
        $config['upload_path']          = './public/upload/organisasi/';
        ...
        if (!$this->upload->do_upload('file_image')) {
            ...
            $query = $this->db->insert("organisasi", $data2);
            $response = array(
                'status' => 'success',
    ...
    public function delete_organisasi()
    {
        ...
        $value = $this->db->where('id', $id)->get('organisasi')->row();
        $existing_image_path = './public/upload/organisasi/' . $value->file_img;
        unlink($existing_image_path);
        $query = $this->db->where('id', $id)->delete('organisasi');
        if ($query) {
            $response = array(
                'status' => 'success',
